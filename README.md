# 👋 Hi, my name is Andreas Alexander Rovelt

I enjoy coding and building projects using different technologies.
I mainly work with **backend, scripting, frontend, and game development**.

---

## Technologies I Work With

![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge\&logo=openjdk\&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge\&logo=javascript\&logoColor=black)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge\&logo=react\&logoColor=61DAFB)
![HTML](https://img.shields.io/badge/HTML-E34F26?style=for-the-badge\&logo=html5\&logoColor=white)
![Lua](https://img.shields.io/badge/Lua-2C2D72?style=for-the-badge\&logo=lua\&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-003B57?style=for-the-badge\&logo=database\&logoColor=white)
![CSS](https://img.shields.io/badge/CSS-1572B6?style=for-the-badge\&logo=css3\&logoColor=white)

---

## Experience

I have worked with:

* ☕ **Java** – applications and backend development
* 🌐 **JavaScript** – web development and scripting
* ⚛️ **React** – frontend development and interactive user interfaces
* 🧱 **HTML** – structuring web pages and frontend layouts
* 🎨 **CSS** – styling web projects
* 🎮 **Lua** – scripting, for example game scripting
* 🛠 **Scripting** – automation and tools
* 🗄 **SQL** – databases and queries

---

## Projects

On my GitHub you can find projects related to:

* 💻 Software development
* 🎮 Game scripting
* 🌐 Web development
* ⚛️ React projects
* ⚙️ Scripts and tools

You can preview my previous projects here:

[Cupcake project](https://projekt1.roneu.dk/)

[FOG project](https://projekt2.roneu.dk/)

[Teamplanner project]((https://projekt3.roneu.dk/)

[Teamplanner rest API](https://teamplanner.roneu.dk/routes)

---

## 📫 Contact

If you want to collaborate or have questions, feel free to reach out.

**Thanks for visiting my profile!**
